...
function ReadingsForm({
  ...
  async function handleRecalculate() {
    if (!sessionId) {
      setSubmitError("No session ID found. Please create a session first.");
      return;
    }
    ...
    try {
      // Submit all rows to the backend one at a time.
      ...
      <div style={{ maxWidth: 960, margin: "0 auto", padding: "48px 32px" }}>
        ...
        <div style={{ background: "var(--color-surface)", border: "1px solid var(--color-border)", borderRadius: "var(--radius)", padding: "20px 24px", boxShadow: "var(--shadow-sm)", marginBottom: 16 }}>
          ...
          <div style={{ display: "flex", gap: 24 }}>
            <CheckboxField id="auto-sequence" label="Auto-select Sequence from UUC Accuracy" checked={autoSelectSequence} onChange={setAutoSelectSequence} />
            <CheckboxField id="auto-points" label="Auto-generate points from UUC range" checked={autoGeneratePoints} onChange={setAutoGeneratePoints} />
          </div>
        </div>
        ...
        <div style={{ background: "#F0F4FF", border: "1px solid #C7D7FF", borderRadius: "var(--radius)", padding: "10px 16px", marginBottom: 16, display: "flex", justifyContent: "space-between", alignItems: "center", fontSize: 13 }}>
          ...
          <button onClick={loadDemoData} style={{ background: "none", border: "none", color: "var(--color-accent)", cursor: "pointer", fontSize: 13, fontWeight: 500 }}>
            Load Demo Data
          </button>
        </div>
              ...
              <tbody>
                {rows.map((row, index) => (
                  <tr key={row.point_number} style={{ background: index % 2 === 0 ? "white" : "#F9FAFB", borderBottom: "1px solid var(--color-border)" }}>
                    ...
                    <td style={tdStyle}><input type="number" value={row.nominal_value} onChange={e => updateRow(index, "nominal_value", e.target.value)} style={inputStyle} /></td>
                    <td style={tdStyle}><input type="number" value={row.measured_value_up} onChange={e => updateRow(index, "measured_value_up", e.target.value)} style={inputStyle} /></td>
                    <td style={tdStyle}><input type="number" value={row.measured_value_down} onChange={e => updateRow(index, "measured_value_down", e.target.value)} style={inputStyle} /></td>
        ...
        <div style={{ display: "flex", gap: 12 }}>
          <button
            ...
            onMouseEnter={e => { if (!isSubmitting) e.currentTarget.style.background = "var(--color-primary-hover)"; }}
            onMouseLeave={e => { if (!isSubmitting) e.currentTarget.style.background = "var(--color-primary)"; }}
          >
            {isSubmitting ? "Saving Readings..." : "Recalculate Metrology Uncertainty"}
          </button>
          <button
...
function CheckboxField({ id, label, checked, onChange }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
      <input type="checkbox" id={id} checked={checked} onChange={e => onChange(e.target.checked)} style={{ accentColor: "var(--color-primary)", width: 14, height: 14 }} />
      <label htmlFor={id} style={{ fontSize: 13, color: "var(--color-text)", cursor: "pointer" }}>{label}</label>
    </div>
  );
}
